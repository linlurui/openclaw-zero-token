...
export function renderOverview(props: OverviewProps) {
  ...
  const handleRestart = () => {
    if (confirm(t("overview.service.restartConfirm"))) {
      props.onServiceRestart();
    }
  };
    ...
    <section class="grid grid-cols-2">
      <div class="card">
        ...
        <div class="row" style="margin-top: 14px;">
          ...
          <button
            class="btn"
            ?disabled=${props.serviceBusy}
            @click=${handleRestart}
          >
            ${
              props.serviceBusy
                ? html`<span class="btn-spinner"></span> ${t("overview.service.restarting")}`
                : t("overview.service.restart")
            }
          </button>
        </div>
        ${
